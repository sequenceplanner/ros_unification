...
#----------------------------------------------------------------------------------------
    ...
    # RECU Scene Master based on Setek RECU ROS Driver specification
...
class recu_smaster():
    ...
    def __init__(self):
        ...
        rospy.init_node('recu_smaster', anonymous=False)
        ...
        # subscribers
        rospy.Subscriber("/unification_roscontrol/recu_unidriver_to_smaster", RecuSPToUni2, self.recuUnidriverCallback)

        # publishers
        self.recu_to_sp_publisher = rospy.Publisher('/unification_roscontrol/recu_smaster_to_unidriver', RecuUniToSP2, queue_size=10)
    ...
    def main(self):
        ...
        self.recu_state = RecuUniToSP2()

        while not rospy.is_shutdown():

            RecuUniToSP2.got_cmd_lock_rsp = self.lock_rsp
            RecuUniToSP2.got_cmd_unlock_rsp = self.unlock_rsp
            RecuUniToSP2.got_cmd_open_gripper = self.open_gripper
            RecuUniToSP2.got_cmd_close_gripper = self.close_gripper
            RecuUniToSP2.robot_not_connected_to_tool = self.robot_not_connected_to_tool
            RecuUniToSP2.robot_connected_to_lf_tool = self.robot_connected_to_lf_tool
            RecuUniToSP2.robot_connected_to_atlas_tool = self.robot_connected_to_atlas_tool
            RecuUniToSP2.robot_connected_to_filter_tool = self.robot_connected_to_filter_tool
            RecuUniToSP2.undefined_connection_detected = self.undefined_connection_detected
            RecuUniToSP2.robot_tool_connection_failure = self.robot_tool_connection_failure
            RecuUniToSP2.ladder_frame_not_connected = self.ladder_frame_not_gripped
            RecuUniToSP2.ladder_frame_connected = self.ladder_frame_gripped
            RecuUniToSP2.ladder_frame_connection_failure = self.ladder_frame_connection_failure
            RecuUniToSP2.pressure_ok = self.pressure_ok
    ...
    def recuUnidriverCallback(self, recu_cmd):
